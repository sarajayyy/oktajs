...
/**
 * @method refresh        - Refresh an existing session for a user
 * @param sessionId       - id of user’s session
 * @param callback        -
 */
OktaAPISessions.prototype.refresh = function(sessionId, callback)
{
    if(sessionId == undefined) throw new Error("A session ID is required when refreshing a session");
    this.request.post("sessions/" + sessionId + "/lifecycle/refresh", null, null, callback);
}
